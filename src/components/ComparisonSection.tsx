...
const ComparisonSection = ({
  ...
  return <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
      ...
      <Card className="border-red-200 shadow-lg overflow-hidden">
        ...
        <CardContent className="p-6">
          <ul className="space-y-4">
            {withoutMembershipItems.map((item, index) => <motion.li key={index} custom={index} initial="hidden" whileInView="visible" viewport={{
            once: true
          }} variants={fadeIn} className="flex items-start gap-3">
      ...
      <Card className="border-green-200 shadow-lg overflow-hidden">
        ...
        <CardContent className="p-6">
          <ul className="space-y-4">
            {withMembershipItems.map((item, index) => <motion.li key={index} custom={index} initial="hidden" whileInView="visible" viewport={{
            once: true
          }} variants={fadeIn} className="flex items-start gap-3">
                <span className="mt-1 flex-shrink-0">{item.icon}</span>
                <p className="text-gray-700">{item.text}</p>
